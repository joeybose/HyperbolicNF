...
class ProbabilisticBinaryTreeDataset(torch.utils.data.Dataset):
    """
    This synthetic dataset comes from the following project:
    https://github.com/pfnet-research/hyperbolic_wrapped_distribution
    """

    def __init__(self, eps, depth=8, data=None):
        self.eps = eps
        self.depth = depth
        if data is None:
    ...
    def __getitem__(self, index):
        ...
        sample = (self.data[index].astype(bool) ^ filter_).astype(self.data.dtype)
        return torch.Tensor(sample), torch.zeros(255)
    ...
    def get_data(self, dtype=np.float32):
        n = 2 ** self.depth - 1

        x = np.fromfunction(lambda i, j: np.vectorize(ProbabilisticBinaryTreeDataset.hasone)(i + 1, j + 1),
                            (n, n), dtype=np.int32).astype(dtype)
        return x


class PbtVaeDataset(VaeDataset):

    def __init__(self, batch_size, *args, **kwargs):
        super().__init__(batch_size, in_dim=255, img_dims=None)

    def _load_synth(self, dataset: ProbabilisticBinaryTreeDataset, train: bool = True) -> DataLoader:
        return DataLoader(dataset=dataset, batch_size=self.batch_size, num_workers=8, pin_memory=True, shuffle=train)

    def create_loaders(self) -> Tuple[DataLoader, DataLoader]:
        train_dataset = ProbabilisticBinaryTreeDataset(eps=0.1, depth=8)
        valid_dataset = ProbabilisticBinaryTreeDataset(eps=0.1, data=train_dataset.data.copy())
        test_dataset = ProbabilisticBinaryTreeDataset(eps=0.1, data=train_dataset.data.copy())
